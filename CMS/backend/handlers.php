...
function handle_login(PDO $pdo): void
{
    ...
    if (!$u || !(int) $u['is_active'] || !password_verify($password, $u['password_hash'])) {
        ...
        if ($userId) {
            log_login($pdo, $userId, 'failed', 'Invalid credentials');
        }
    ...
    log_login($pdo, (int) $u['id'], 'success');
    json_out([
        'ok' => true,
...
function handle_forgot_password(PDO $pdo, array $config): void
{
    ...
    $st = $pdo->prepare('SELECT id, email, full_name, is_active FROM users WHERE email = ? LIMIT 1');
    $st->execute([$email]);
    $user = $st->fetch();

    // Always return a generic success response (prevents account enumeration).
    ...
    $frontendBase = rtrim((string) ($config['frontend_url'] ?? $config['cors_origin'] ?? ''), '/');
    $resetUrl = $frontendBase . '/login?mode=reset&token=' . urlencode($rawToken);
    $fullName = htmlspecialchars((string) ($user['full_name'] ?? 'User'), ENT_QUOTES | ENT_SUBSTITUTE);
    ...
    log_activity($pdo, $userId, 'password_reset_requested', null);
...
function handle_reset_password(PDO $pdo): void
{
    ...
    if ($token === '' || $password === '') {
        json_out(['ok' => false, 'error' => 'Token and new password are required'], 422);
    }
    ...
    log_activity($pdo, $userId, 'password_reset_completed', null);
    json_out(['ok' => true, 'message' => 'Password reset successful. Please login with your new password.']);
}
